...
The single-panel demonstration of \Cref{sec:demonstration} shows what the \texttt{vol-eval} battery looks like when applied to one well-documented horse race. The natural question is how the findings generalize. This section reports the result of applying \texttt{vol-eval} to a sample of 30 published volatility-forecasting comparison papers from 2018--2025.

We organize the section around three layered findings: (i) reproducibility-disclosure rates in the sample, (ii) the use of formal significance tests in the original papers, and (iii) for the subset we can reproduce, the survival of headline ``wins'' under \texttt{vol-eval}.
...
This is the strongest empirical case for the \texttt{vol-eval} package. The volatility-forecasting subfield publishes ``X beats Y'' claims at scale without significance testing. The package exists to remove the labor cost of doing the right thing: each test is a one-line API call.
...
\Cref{tab:phase2_appendix} provides one row per reproduced paper with the key vol-eval outputs: paper-claimed winner, our best-by-QLIKE model in the reproduction, DM $p$-value of the comparison the paper makes, and 90\% MCS survivor count. Multi-asset reproductions (P09 across four Colombian ADRs, P12 across six cryptocurrencies, P15 across five cryptocurrencies, P27 across two FX pairs) report the asset that is most damning to the paper's claim, i.e., the smallest DM $p$-value across assets, equivalently the asset where the test most clearly rejects the paper's claim of a winner. The per-asset detail is in the per-paper JSON output files. The reported $p$-value is mechanically verified against the per-paper \texttt{voleval\_result.json} by the audit script in this paper's repository.

\begin{table}[H]
\centering
\caption{Per-paper vol-eval outcomes for the 14 reproducible papers. ``DM~$p$'' is the Diebold-Mariano $p$-value for paper-claimed winner versus paper-claimed runner-up at $h=1$. ``MCS surv/tot'' is the number of models retained by the 90\% MCS over the total in the comparison. Asterisk (*) marks reproductions with substantial library/feature substitutions detailed in \Cref{sec:phase2_caveats}. ``rev'' marks rows where the DM test rejects in the direction \emph{opposite} to the paper's claim.}\label{tab:phase2_appendix}
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{2.0cm}p{0.8cm}p{3.3cm}p{2.9cm}p{1.5cm}p{1.2cm}p{1.5cm}}
\toprule
